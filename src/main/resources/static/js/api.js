...
async function listarCajas() {
    const res = await fetchAuth(`${BASE_URL}/caja`);
    return res.json();
}

async function crearCaja(montoInicial, departamentoId) {
    const params = new URLSearchParams({ montoInicial, departamentoId });
    const res = await fetchAuth(`${BASE_URL}/caja?${params}`, { method: "POST" });
    if (!res.ok) throw new Error("Error al crear la caja");
    return res.json();
}

async function actualizarCaja(id, nuevoMonto) {
    const res = await fetchAuth(`${BASE_URL}/caja/${id}?nuevoMonto=${nuevoMonto}`, { method: "PUT" });
    if (!res.ok) throw new Error("Error al actualizar la caja");
    return res.json();
}

async function eliminarCaja(id) {
    const res = await fetchAuth(`${BASE_URL}/caja/${id}`, { method: "DELETE" });
    if (!res.ok) throw new Error("Error al eliminar la caja");
}

async function listarMovimientos() {
    const res = await fetchAuth(`${BASE_URL}/movimientos`);
    return res.json();
}

async function listarMovimientosPorCaja(cajaId) {
    const res = await fetchAuth(`${BASE_URL}/movimientos/caja/${cajaId}`);
    return res.json();
}

async function registrarMovimiento(cajaId, tipo, monto, descripcion) {
    const params = new URLSearchParams({ cajaId, tipo, monto, descripcion });
    const res = await fetchAuth(`${BASE_URL}/movimientos?${params}`, { method: "POST" });
    if (!res.ok) throw new Error("Error al registrar movimiento");
    return res.json();
}

async function listarUsuarios() {
    const res = await fetchAuth(`${BASE_URL}/usuarios`);
    return res.json();
}
...
async function actualizarUsuario(id, datos) {
    const res = await fetchAuth(`${BASE_URL}/usuarios/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
...
async function eliminarUsuario(id) {
    const res = await fetchAuth(`${BASE_URL}/usuarios/${id}`, { method: "DELETE" });
    if (!res.ok) throw new Error("Error al eliminar usuario");
}

async function listarGastos() {
    const res = await fetchAuth(`${BASE_URL}/gastos`);
    return res.json();
}

async function listarMisGastos(usuarioId) {
    const res = await fetchAuth(`${BASE_URL}/gastos/mis-gastos/${usuarioId}`);
    return res.json();
}

async function registrarGasto(gasto, cajaId, presupuestoId) {
    const params = new URLSearchParams({ cajaId, presupuestoId });
    const res = await fetchAuth(`${BASE_URL}/gastos?${params}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function aprobarGasto(id) {
    const res = await fetchAuth(`${BASE_URL}/gastos/${id}/aprobar`, { method: "PUT" });
    if (!res.ok) throw new Error("Error al aprobar gasto");
    return res.json();
}

async function rechazarGasto(id) {
    const res = await fetchAuth(`${BASE_URL}/gastos/${id}/rechazar`, { method: "PUT" });
...
async function subirComprobante(id, archivo) {
    ...
    const res = await fetchAuth(`${BASE_URL}/gastos/${id}/comprobante`, {
        method: "POST",
        body: formData
    });
...
async function listarPresupuestos() {
    const res = await fetchAuth(`${BASE_URL}/presupuesto`);
    return res.json();
}

async function listarDepartamentos() {
    const res = await fetchAuth(`${BASE_URL}/presupuesto/departamentos`);
    return res.json();
}

async function crearDepartamento(nombre) {
    const res = await fetchAuth(`${BASE_URL}/presupuesto/departamento?nombre=${nombre}`, {
        method: "POST"
    });
...
async function crearPresupuesto(departamentoId, mes, presupuestoMensual) {
    ...
    const res = await fetchAuth(`${BASE_URL}/presupuesto?${params}`, { method: "POST" });
    if (!res.ok) throw new Error("Error al crear presupuesto");
    return res.json();
}

async function registrarConsumo(presupuestoId, monto) {
    const params = new URLSearchParams({ presupuestoId, monto });
    const res = await fetchAuth(`${BASE_URL}/presupuesto/consumo?${params}`, { method: "POST" });
